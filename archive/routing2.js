...
import {createHistory} from "@iosio/history";
...
const preparePath = (pathOrUseWindowPathIfTrue) => {
    const p = pathOrUseWindowPathIfTrue === true ? window.location.pathname : pathOrUseWindowPathIfTrue;
    if (p === '/') return "/";
...
let lastPath = preparePath(true);


const canRoute = (path) => accessiblePaths.includes(path || preparePath(true));


const getRedirectPath = ({noMatchRedirectTo, root}) => {
    //assuming we cant route to the existing base path

    let redirectTo = '/';
    //is there is a default path provided? and can we go there?
    if (noMatchRedirectTo && canRoute(noMatchRedirectTo)) {
    ...
    else if ((preparePath(true) !== lastPath) && canRoute(lastPath)) {
        redirectTo = lastPath;
...
const tryRoute = (path, backups, noReplace) => {
    if (!canRoute(path)) {
        const tryToRedirectTo = getRedirectPath(backups || {});
        if (!noReplace) {
            history.replace(tryToRedirectTo);
        }
        return {ok: false, path: tryToRedirectTo}
    } else {
        return {ok: true, path}
    }
};
...
export const gatherChildren = (children, isAuthenticated) => {
    accessiblePaths = [];
    excludedPaths = [];
    componentPathMap = {};
    flattenChildren(children).forEach((child) => {
        ...
        if (pathName) {
            ...
            if (!excluded) {
                accessiblePaths.push(pathName);
                componentPathMap[pathName] = child;
            } else {
                excludedPaths.push(pathName);
            }
        }
...
const getMatchingComponent = (pn) => componentPathMap[pn] || null;


export class Router2 extends React.Component {

    constructor(props) {
        super(props);
        this.state = {CurrentRoute: null};

        const {children, isAuthenticated, noMatchRedirectTo, root} = props;

        gatherChildren(children, isAuthenticated);

        tryRoute(preparePath(true), {noMatchRedirectTo, root});

        this.state = {
            CurrentRoute: getMatchingComponent(preparePath(true))
        };
...
const Subscription = () => {
    ...
    const notify = function () {
        let current = subs;
        for (let i = 0; i < current.length; i++) current[i].apply(undefined, arguments);
    };
...
class Router extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        const {isAuthenticated, onLogoutPath, onLoginPath} = this.props;
...
class Link extends React.Component {
    constructor(props) {
        super(props);
        this.state = history.getLocation();
        this._isMounted = false;
        this.unlisten = history.listen((location) => {
            this._isMounted && this.setState({...location});
    ...
    render() {
        const {render, children, to, href, ...rest} = this.props;
        const place = to || href;
        return (
            <a href={place}
               onClick={(e) => {
